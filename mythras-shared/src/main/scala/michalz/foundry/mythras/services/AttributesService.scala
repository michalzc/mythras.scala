...
trait AttributesService:
  ...
  def calculateActionPoints(int: Int, dex: Int): Int = ActionPointsBase.calculate(int + dex)
  ...
  def calculateMaxHitPointsPerLocation(con: Int, siz: Int, location: BodyLocation): Int =
  ...
  def calculateInitiativeMod(int: Int, dex: Int): Int = scala.math.round((int + dex) / 2.0).toInt
