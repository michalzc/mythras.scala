...
trait DamageModifierService:
  ...
  /** Get damage modifier string based on attribute value.
    ...
    *   Attribute value
    * @return
    *   Damage modifier in dice notation
    */
  ...
  /** Calculate damage modifier for steps beyond the predefined table.
    *
    * @param step
    *   The high damage step to calculate
    * @return
    *   Damage modifier in dice notation
    */
  private def calculateHighStep(step: Int): String =
    ...
    val elements =
      if diceIndex >= 0 then List(s"+${multiplier}d10", SmallerDice(diceIndex))
      else List(s"+${multiplier}d10")
